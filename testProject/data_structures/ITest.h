...
public:
	int _FCCount;
	int _FCCombinedCount;
	//Node* _uselessNode;

public:
	static final int _ANS_TRUE		= (INT_MIN+4); //by Zhiyu
	static final int _ANS_FALSE		= (INT_MIN+5);	//by Zhiyu

protected:

	static final int _MAX_THREADS	= 1024;
	static final int _NULL_VALUE	= 0; //I changed it to (INT_MIN+10) for linked lists. 0 works for other data structures
	static final int _DEQ_VALUE		= (INT_MIN+2);
	static final int _OK_DEQ		= (INT_MIN+3);

	static final int _OP_ADD		= (INT_MIN+6); //by Zhiyu
	static final int _OP_DEL		= (INT_MIN+7); //by Zhiyu
	static final int _OP_CONTAIN	= (INT_MIN+8); //by Zhiyu
	...
	//helper function -----------------------------
		...
		inline void thread_wait(final int iThread, final boolean is_short=false) { 
#if 0
			//why sleep? It slows down threads too much!
			if(_NUM_THREADS < 8) {
...
public:
	...
	struct SlotInfo {
		...
		SlotInfo* volatile	_next;			//when null not connected (this is not true after my modification. --Zhiyu)
		void*				_custem_info;

		bool volatile		_is_slot_linked;	//I add these vars to make flat combining more understandable --- by Zhiyu
		bool volatile		_is_request_valid;	//
		int volatile		_return_val; 		//
		int volatile		_op; 				//
		RNode* volatile		_rNode;				//real Node for MyFCQueue.h
...
protected:
	...
	SlotInfo* get_new_slot() {
		...
		my_slot->_req_ans = _NULL_VALUE; //
		my_slot->_is_slot_linked = true;//
		my_slot->_time_stamp = _timestamp;//
	...
	void enq_slot(SlotInfo* p_slot) {
		p_slot->_is_slot_linked = true;//
		p_slot->_time_stamp = _timestamp;//
